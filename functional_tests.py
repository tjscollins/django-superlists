...
class NewVisitorTest(unittest.TestCase):
    ...
    def test_can_start_a_list_and_retrieve_it_later(self):
        ...
        self.assertIn('1: Buy new swingline stapler', [row.text for row in rows])
        ...
        rows = table.find_elements_by_tag_name('tr')
        self.assertIn('1: Buy new swingline stapler', [row.text for row in rows])
        self.assertIn('2: Burn down office and collect insurance money', [row.text for row in rows])
